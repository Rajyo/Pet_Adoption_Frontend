    ...
    <View style={{ minHeight: "100%", paddingHorizontal: 20 }}>
      {
        (petLiked && petLiked.length > 0)
          ? petLiked?.map((item: any) => (
            <Link key={item._id} href={{ pathname: '/(components)/(explore)/PetProfile', params: { _id: item._id, ageInWeeks: item.ageInWeeks, petBehaviour: item.petBehaviour, breed: item.breed, gender: item.gender, petInfo: item.petInfo, location: item.location, pic: item.pic as any, name: item.name, likes: item.likes as any, typeOfPet: item.typeOfPet } }} >

              <View style={{ padding: 10, marginVertical: 15, borderColor: "#cccccc", borderWidth: 1, borderRadius: 10, shadowColor: "#cccccc", shadowOffset: { width: 5, height: 5 }, shadowRadius: 5, shadowOpacity: 0.8, gap: 10, width:"100%"}}>
